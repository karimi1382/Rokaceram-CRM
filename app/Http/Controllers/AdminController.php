...
foreach ($completedHavale as $havale) {
    $month = Jalalian::fromCarbon($havale->created_at)->getMonth();  // استخراج ماه شمسی
    ...
    public function allHavale(HavaleSyncService $havaleSync)
{
    ...
    foreach ($uniqueRequests as $havaleNumber => $requestsForHavale) {
        ...
        try {
            $havaleData = DB::connection(name: 'sqlsrv')->select("SELECT * FROM vw_HavaleData WHERE havale = ?", [$request->havale_number]);
